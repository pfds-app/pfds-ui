 ...
 *
 * @export
 * @interface UserGenderStat
 */
export interface UserGenderStat {
  /**
   *
   * @type {number}
   * @memberof UserGenderStat
   */
  year: number;
  /**
   *
...
export const UsersApiAxiosParamCreator = function (
  ...
  return {
    ...
    getUserGenderStats: async (
      ...
      const localVarPath = `/users/genders/stats`;
      // use dummy base URL string because the URL constructor only accepts absolute URLs.
      const localVarUrlObj = new URL(localVarPath, DUMMY_BASE_URL);
      let baseOptions;
      if (configuration) {
  ...
  return {
    ...
    async getUserGenderStats(
      ...
      (
        axios?: AxiosInstance,
        basePath?: string
      ) => AxiosPromise<Array<UserGenderStat>>
    > {
      const localVarAxiosArgs =
        await localVarAxiosParamCreator.getUserGenderStats(
          fromDate,
          endDate,
          options
        );
      const localVarOperationServerIndex = configuration?.serverIndex ?? 0;
      const localVarOperationServerBasePath =
        operationServerMap["UsersApi.getUserGenderStats"]?.[
          localVarOperationServerIndex
        ]?.url;
      return (axios, basePath) =>
        createRequestFunction(
...
export const UsersApiFactory = function (
  ...
  return {
    ...
    getUserGenderStats(
      ...
      return localVarFp
        .getUserGenderStats(fromDate, endDate, options)
        .then((request) => request(axios, basePath));
    },
    /**
     *
